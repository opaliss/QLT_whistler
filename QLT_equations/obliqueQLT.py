...
def sum_bessel(lambda_, omega, k_par, alpha_c_par, n_max=20, n_factor=0, include_Z=True):
    ...
    for n in range(-n_max, n_max + 1):
        xi = (omega - n) / np.abs(k_par) / alpha_c_par
        if include_Z:
            Z_ = Z(z=xi)
            sol += I(m=n, Lambda=lambda_) * Z_ * (n ** n_factor)
        else:
...
def dKperpdt(E_vec, omega_pe, alpha_c_par, alpha_c_perp, n_c, k_par, k_perp, omega_vec, dk, omega_0):
    """
    ...
    for ii in range(len(k_par)):
        if omega_vec[ii] != omega_0:
            ...
            term1_bessel = sum_bessel(lambda_=lambda_, omega=omega_vec[ii], k_par=k_par[ii],
                                      alpha_c_par=alpha_c_par, n_factor=1)

            term_1 = (omega_vec[ii] / (k_par[ii] * alpha_c_par)) * term1_bessel

            # second term involving n^{2}
            term_2 = (anisotropy_term / k_par[ii] / alpha_c_par) * sum_bessel(lambda_=lambda_,
                                                                              omega=omega_vec[ii],
                                                                              k_par=k_par[ii],
...
def dKpardt(E_vec, omega_pe, alpha_c_par, alpha_c_perp, n_c, k_par, k_perp, omega_vec, dk, omega_0):
    """
...
def dTperpdt(E_vec, omega_pe, alpha_c_par, alpha_c_perp, k_par, k_perp, omega_vec, dk, omega_0, n_max=5):
    """
...
def dTpardt(E_vec, omega_pe, alpha_c_par, alpha_c_perp, k_par, k_perp, omega_vec, dk, omega_0, n_max=2):
    """
    ...
    for ii in range(len(k_par)):
        if omega_vec[ii] != omega_0:
            ...
            for n in range(-n_max, n_max + 1):
                xi_n = ((omega_vec[ii] - n) / k_par[ii] / alpha_c_par).real
                exp = np.exp(-xi_n ** 2)
                I_term = I(Lambda=lambda_, m=n)
                omega_term = omega_vec[ii].real + n * anisotropy_term
                next_term = I_term * omega_term * xi_n * exp
                term_1 += next_term
            sol[ii] = E_vec[ii] / k2 * term_1 * k_perp[ii]
    # integrate over all k's
